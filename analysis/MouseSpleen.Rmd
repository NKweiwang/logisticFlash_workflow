...
factor 1 is mainly for the total missing rate

```{r}
i = 1
par(mfrow = c(2,2), mar = c(5,4,4,2) - 1.9)
plot(as.numeric(as.factor(cell_labels$sequencing_batch[col_index])),MissPattern$f[,i])
plot(as.numeric(as.factor(cell_labels$amplification_batch[col_index])),MissPattern$f[,i])
plot(as.numeric(as.factor(cell_labels$group_name[col_index])),MissPattern$f[,i])
